...
<div class="sale-detail-wrapper">
    <div class="container py-3" style="max-width: 900px;">
        ...
        <div class="mb-4 d-flex justify-content-between align-items-center flex-wrap gap-3">
            ...
            <a href="{{ route('penjualan.index') }}" class="btn-gunmetal-back">
        ...
        <div class="sale-summary-card">
            <div class="row g-3">
                <div class="col-md-4">
                    ...
                    <div class="sale-info-value">{{ optional($sale->user)->name ?? 'Sistem' }}</div>
                </div>
                <div class="col-md-4">
                    <div class="sale-info-label mb-1"><i class="bi bi-calendar-event-fill me-1 text-warning"></i> Tanggal Transaksi</div>
                    <div class="sale-info-value">{{ $sale->created_at->translatedFormat('d-m-Y H:i:s') }}</div>
                </div>
                <div class="col-md-4">
                    <div class="sale-info-label mb-1"><i class="bi bi-cash-stack me-1 text-success"></i> Total Pembayaran</div>
                    <div class="sale-info-value text-info">Rp {{ number_format($sale->total_pembayaran, 0, ',', '.') }}</div>
                </div>
            </div>
        </div>
            ...
            <div class="table-responsive">
                <table class="table table-silver align-middle">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="text-center fw-bold" style="color: #94a3b8 !important;">{{ $i++ }}</td>
                            <td class="text-center">
                                @if($item->produk && $item->produk->foto)
                                    <img src="{{ asset('storage/' . $item->produk->foto) }}" class="table-product-img" alt="Foto Produk">
                                @else
                            ...
                            <td>
                                <span class="fw-bold text-white">{{ optional($item->produk)->nama ?? 'Produk Dihapus' }}</span>
                            </td>
                            <td class="text-end fw-bold text-info">
                                Rp {{ number_format(optional($item->produk)->harga_jual ?? 0, 0, ',', '.') }}
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
